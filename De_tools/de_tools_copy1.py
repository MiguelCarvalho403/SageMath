...
def is_trivially_separable(splitted_pde, sep_variables, show_log):# Verify whether the PDE is separated into terms that contain only one variable, return True or False
    flag=True
    #print("splitted_pde: ",splitted_pde)
...
def list_of_terms(splitted_pde, sep_variables): # Group the terms that contain the same variable
    #print(sep_variables)
    #print([sum([q for q in splitted_pde if v in q.variables()]) for v in sep_variables])
...
def separate_equations(partially_separated_pde, funcs, sep_variables, sep_constant, check, show_log):
    equations = []
    #print("partially_separated_pde: ",partially_separated_pde)
...
def check_separated(partially_separated_pde, equations, sep_constant, show_log):
    ...
    #print('',sum(partially_separated_pde), '\n', sum(initial_eqs))
    check_result(sum(partially_separated_pde), sum(initial_eqs), 'separate_equations test 2: ', show_log)
    pass
...
def partial_common_factor(partially_separated_pde, sep_variables, check, show_log):
    ...
    common_factor = get_common_factor(eq_operands(partially_separated_pde[0])) # Get common factor from mixed terms
...
#        initial_de = initial_de.lhs()-initial_de.rhs()
    from sage.symbolic.function import SymbolicFunction # add
    
    if isinstance(fun, SymbolicFunction): # add
        fun = fun(variable) # add
...
def get_de_coefficients(initial_de, fun, variable):
    ...
    if initial_de.is_relational(): # verificar se é uma equação
        ...
        de = initial_de
    #print(fun, variable, type(fun))
    return {k:formal_diff(de,diff(fun, variable, k)).expand().factor() for k in range(3)}
    #return {k:formal_diff(de,diff(fun, variable, k)).expand().factor() for k in range(3)} original
...
def check_result(expression_1, expression_2, step_message='are equals? ', show_log=True): # flag2
    ...
    difference = (expression_1-expression_2).canonicalize_radical() #flag ch
    if difference:
        message = Fore.RED + 'failed' + Fore.RESET
        print(step_message + message)
    else:
...
def _separate_pde(pde,
    ...
    ***
    ...
    if is_trivially_separable(partially_separated_pde, sep_variables, show_log): # Check whether EDP can be separated, in other words, there is no mix-terms
