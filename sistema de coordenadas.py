def coordenadas(ax_size = (-2,2), plane = True, opacity = 0.7, grad = 1):
    # grad: graduação a qual a malha é subdividida, por padrão 1 unidade
    # ax_size: intervalo que a malha se extenderá, sempre se extenderá como um quadrado
    u, v = var('y, z')
    ...
    labels = text3d('x',1.1*vector([ax_size[1],0,0]),fontsize= str(250)+'%')
    
    y = parametric_plot((0, u, 0), u_,  opacity=1, color='yellow')
    y += vector([0,ax_size[1],0]).plot(color = 'yellow')
    labels += text3d('y',1.1*vector([0,ax_size[1],0]),fontsize= str(250)+'%')
    
    z = parametric_plot((0, 0, u), u_,  opacity=1) # Blue
    z += vector([0,0,ax_size[1]]).plot(color = 'blue')
    labels += text3d('z',1.1*vector([0,0,ax_size[1]]),fontsize= str(250)+'%')
...
u = var('u')
coordenadas(ax_size = (-2,2)) + parametric_plot((2,0,u), (u,-2,2), color = 'blue')
